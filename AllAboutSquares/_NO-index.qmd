...
## Squares {background-image="img/SquaresBG.jpg"}
...
```{r}
ggplot(OregonSalaries) + aes(x=Gender, y=Salary, fill=Gender) + geom_violin() 
```
...
## A Big Black Box {background-image="img/Dev2.png"}
...
Because the $e_{i}$ being normal means implies that their squares are $\chi^2$[^This is why the various results in prop.test are reported as X-squared.] and further that the ratio of their mean-squares are $F$.

As an aside, the technical definition of $t$[^I mentioned this fact previously when introducing t (defined by degrees of freedom and with metric standard error.)] is a normal divided by the square root of a $\chi^2$ so $t$ and $F$ must be equivalent with one degree of freedom in the numerator.
...
```{r, warning=FALSE, message=FALSE}
Semiconductors %>% ggplot() + aes(x=units, y=TotCost4) + geom_point() + geom_smooth(method = "lm") + geom_smooth(method = "loess", color="red")  + labs(x="Units", y="Total Cost", title="Semiconductors")
```
...
PowerPlant %>% ggplot() + aes(x=units, y=TotCost3, color=PlantFac) + geom_point() + geom_smooth(method = "lm") +  labs(x="Units", y="Total Cost", title="Power Plant")
```
